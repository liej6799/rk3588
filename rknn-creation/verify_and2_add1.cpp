// Verify 2-And + 1-Add parallel model: 6 inputs (4 bool + 2 float), 3 outputs
#include <cstdint>
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
int main(int argc,char**argv){
  ...
  // a,b,c,d bool; x,y float
  int8_t a[4]={1,0,1,1}, b[4]={1,1,0,1};
  int8_t c[4]={0,1,1,0}, d[4]={1,1,0,1};
  ...
  ret=rknn_outputs_get(ctx,3,outs,NULL);printf("outputs_get: %d\n",ret);if(ret<0)return 1;

  int bad=0;
  int8_t*o1=(int8_t*)outs[0].buf;
  int8_t*o2=(int8_t*)outs[1].buf;
  float*o3=(float*)outs[2].buf;
  ...
  rknn_outputs_release(ctx,3,outs);rknn_destroy(ctx);
  return bad?1:0;
}
