                               ...
                               int *type_bytes)
{
  ...
  *type_bytes = npy_data.word_size;
  ...
  unsigned char *data = (unsigned char *)malloc(1200);
  if (!data)
  {
    return NULL;
  }
...
// data = reinterpret_cast<unsigned char*>(test);
  ...
  *input_size = 240;
  ...
  std::cout << "Type of b: " << typeid(test).name() << std::endl;


  float* Pf = test;  // Pf now points to the first element of A30_f
 
  return reinterpret_cast<unsigned char*>(float_data);
}
                                    ...
                                    rknn_custom_op_tensor* outputs, uint32_t n_outputs)
{
  ...
  for (int ridx0 = 0; ridx0 < 5; ridx0++) {
    float val0 = *(in_data_0+ridx0);
    float val1 = *(in_data_1+ridx0);
    *(out_data_0+ridx0) = (val0+val1);
  }
