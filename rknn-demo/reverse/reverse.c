...
// Function to perform a hexdump of a memory region
void hexDump(const char *desc, const void *addr, int len, int perLine) {
    int i;
    unsigned char buff[17]; // Buffer for ASCII representation
    ...
    // Process each byte
    for (i = 0; i < len; i++) {
        // New line and offset at the start of each line
        if ((i % perLine) == 0) {
            if (i != 0) {
                printf("  %s\n", buff); // Print ASCII from previous line
            }
            printf("  %04x ", i); // Print offset
        }

        // Print hex value of the current byte
        printf("%02x ", pc[i]);

        // Store ASCII character (or '.' for non-printable)
        if ((pc[i] < 0x20) || (pc[i] > 0x7e)) {
            buff[i % perLine] = '.';
        } else {
            buff[i % perLine] = pc[i];
        }
        buff[(i % perLine) + 1] = '\0'; // Null-terminate for printing
    }

    // Pad with spaces for alignment if the last line is not full
    while ((i % perLine) != 0) {
        printf("   ");
        i++;
    }

    printf("  %s\n", buff); // Print ASCII for the last line
}

// Add this function to your reverse.c file
void* mapPhysicalAddress(uint64_t physical_addr, size_t size) {
    // First, we need to find the handle for this object address
    // This requires using the RKNPU memory management interface
    
    printf("Attempting to map physical address: 0x%lx\n", physical_addr);
    
    // Method 1: Try to use /dev/mem (requires root)
    int mem_fd = open("/dev/mem", O_RDWR | O_SYNC);
    if (mem_fd >= 0) {
        printf("Using /dev/mem for mapping\n");
        
        size_t page_size = sysconf(_SC_PAGE_SIZE);
        uint64_t page_addr = physical_addr & ~(page_size - 1);
        size_t offset = physical_addr - page_addr;
        
        printf("Page size: %zu, Page addr: 0x%lx, Offset: %zu\n", 
               page_size, page_addr, offset);
        ...
        if (mapped != MAP_FAILED) {
            void* result = (char*)mapped + offset;
            printf("Successfully mapped to virtual address: %p\n", result);
            return result;
        } else {
            printf("mmap failed: %s\n", strerror(errno));
        }
    } else {
        printf("Cannot open /dev/mem: %s (try running as root)\n", strerror(errno));
    }
    
    // Method 2: Try to find the object in existing mappings
    printf("Physical address 0x%lx might be already mapped in your process\n", physical_addr);
    printf("Check if this matches any of your allocated objects\n");
    
    return NULL;
}

void unmapPhysicalAddress(void* virtual_addr, size_t size) {
    if (virtual_addr) {
        size_t page_size = sysconf(_SC_PAGE_SIZE);
        uintptr_t page_addr = (uintptr_t)virtual_addr & ~(page_size - 1);
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < 1; i++) {
    ...
    void *map = mmap(NULL, 1024, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0x100001000);
    // Get first 1024 bytes from map and print as hex
    

    uint64_t map_data[256];
