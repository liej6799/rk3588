...
// Load test data for float32 tensors
static unsigned char *load_float32_data(rknn_tensor_attr *input_attr, int *input_type, int *input_size,
    int *type_bytes, int index)
{
    // Create test data: input 0 = [5.5,5.5,5.5,...], input 1 = [3.2,3.2,3.2,...]
    float* test = (float*)malloc(10 * sizeof(float));
    ...
    // Copy the data
    memcpy(data, test, 10 * sizeof(float));
    
    printf("Input %d float32 data: ", index);
    for (int i = 0; i < 10; ++i) {
        printf("%.1f ", data[i]);
    }
    printf("\n");
...
int main() {
    printf("Float32 Addition Test\n");
    printf("Expected: [5.5,5.5,5.5,...] + [3.2,3.2,3.2,...] = [8.7,8.7,8.7,...]\n\n");
    
    // Load model file
    const char* model_path = "../float32add.rknn";
    ...
    // Load input data
    for (int i = 0; i < io_num.n_input; i++) {
        input_data[i] = load_float32_data(&input_attrs[i], &input_type[i], 
